...
\hline
Hạ tầng & \path{01_gcp_vm_specs.png}, \path{02_gcp_vm_ip_addr.png}, tool gate và PVC/database output trong \path{.agents/evidence/README.md}. \\
\hline
ArgoCD/GitOps & \path{06_argocd_apps_overview.png}, \path{07_validate_gitops_passed.txt}, \path{11_gitops_structure_review.txt}. \\
\hline
Staging release & \path{21_jenkins_v010_gate_blocked.txt}, \path{21b_jenkins_v011_buildx_missing.txt}, \path{21c_jenkins_v011_mixed_tag_promote_fail.txt}, \path{21d_jenkins_v012_release_success.txt}. \\
\hline
...
\hline
\path{img/platform/01_gcp_vm_specs.png} & \path{evidence/01_gcp_vm_specs.png} & VM 32GB RAM dùng làm Jenkins agent và K3s node. \\
\hline
...
\hline
P0 & Jenkins main build success và release \texttt{v0.1.2}. & Chứng minh pipeline chạy thật thay vì chỉ có log text. \\
\hline
...
\hline
P0 & Docker Hub tags cho commit SHA và release tag. & Chứng minh branch/main/release image tagging đúng đề bài. \\
\hline
...
\begin{itemize}
    ...
    \item Payment staging Liquibase checksum: release \texttt{v0.1.2} đã promote/deploy đúng image, nhưng service payment có lỗi migration checksum khi restart trên DB bền. Đây là vấn đề runtime/application migration, không phải lỗi GitOps release flow.
