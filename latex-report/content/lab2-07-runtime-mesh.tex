...
\caption{Scaffold ảnh pod có workload container và Istio sidecar \texttt{READY 2/2}}
\end{figure}
...
\caption{Scaffold ảnh curl allow: \texttt{tax} gọi \texttt{location}}
\end{figure}

\begin{figure}[H]
\centering
\reportimg{img/mesh/authorization_deny_cart_location.png}
\caption{Scaffold ảnh curl deny: \texttt{cart} gọi \texttt{location} bị Envoy RBAC 403}
\end{figure}

\begin{figure}[H]
\centering
\reportimg{img/mesh/retry_policy_evidence.png}
\caption{Scaffold ảnh retry policy hoặc log retry cho service trong critical path}
\end{figure}
