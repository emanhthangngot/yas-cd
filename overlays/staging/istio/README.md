# Staging Istio Overlay

This folder groups the staging service-mesh and auth-routing patches by function.

- `namespace.yaml`: enables Istio sidecar injection for the `staging` namespace.
...
- `keycloak-oauth-patch.yaml`: configures BFF OAuth endpoints and redirect URIs for Keycloak through the staging NodePort host.
